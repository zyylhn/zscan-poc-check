...
//并发执行内置poc
func CheckBuiltinPoc(req *http.Request, Pocs embed.FS, workers int, pocname string) {
	...
	for i := 0; i < workers; i++ {
		wg.Add(1)
		go runPocExec(tasks,&wg)
	}
	wg.Wait()
}

func CheckExternalPoc(req *http.Request,pocdir string,workers int,pocname string)  {
...
func CheckSinglePoc(req *http.Request,pocpath string)  {
...
func runPocExec(tasks chan Task,wg *sync.WaitGroup)  {
	defer wg.Done()
